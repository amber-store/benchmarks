# Benchmark run `20260912T193856Z-smoke`

Smoke verification on bld1: every backend and every scenario in minutes, 2 repetitions, seed 20260913. A correctness gate — the smoke sizes are far too small to compare backends on.
...
| | |
|---|---|
| started (UTC) | 2026-09-12T19:38:56Z |
| wall clock | 2.2 min |
| profile | `smoke` — ~35 MiB per corpus generation, 3 generations, 14 commits, the 5.7 MiB Nix fixture closure, 1 repetition by default. Minutes. |
| repetitions | 2 per (scenario, backend) |
| seeds | corpus and fixtures `20260913`, randomised backend order `6624854655743516624` |
| worker threads offered | 32 |
| Amber segment size | 8.00 MiB (both cores, explicitly) |
...
| load average at start | 2.81, 2.42, 2.40 (1/5/15 min) |
...
| transfer measurement | every backend's S3 traffic is forwarded verbatim through a counting gateway at http://127.0.0.1:39091; requests are counted by method and bodies by direction, including retries. The harness's own listing and cleanup calls bypass it. |
